<template>
  <q-page style="display:flex">
    ...
    <div style="flex:1;display:flex;flex-direction:column">
      ...
      <!-- Tabs -->
      <div style="flex:0 0">
        <q-tabs underline-color="transparent" swipeable v-model="selectedTab" color="dark">
      ...
      <q-scroll-area style="flex:1"
        :style="{ background: (selectedTabIndex>=0)?tabs[selectedTabIndex].tabColor:'red' }">
        <template v-for="(image, index) in tabs[selectedTabIndex].images">
          <q-card
            v-if="image.hidden === false"
            :draggable="image.unavailable?false:true"
            :disabled="image.unavailable?true:false"
            @dragstart.native="drag(selectedTabIndex, index, $event)"
            inline
            class="q-ma-sm bg-white"
            style="width: 150px"
            :key="selectedTabIndex + '-' + index" >
              <q-card-media style="padding:2px">
                  <img :src="image.url" style="user-drag: none">
